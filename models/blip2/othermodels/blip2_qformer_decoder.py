...
@register_model("speech_qformer_decoder")
class Blip2Qformer(Blip2Base):
    ...
    def forward(self, samples):
        ...
        lm_output = self.Qformer.cls(query_output[0])
        # breakpoint()
        loss_fct = CrossEntropyLoss()  # -100 index = padding token
        masked_lm_loss = loss_fct(
                lm_output.view(-1, self.Qformer.config.vocab_size), labels.view(-1)
            )
        
        return BlipOutput(
            loss=loss_itc + loss_itm + masked_lm_loss,
            loss_itc=loss_itc,
            loss_itm=loss_itm,
            loss_lm=masked_lm_loss,
        )
...
@register_model_architecture(model_name="speech_qformer_decoder", arch_name="speech_qformer_decoder")
