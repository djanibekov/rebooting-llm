...
@register_model("sparqle_finetune_llm")
class SparQLeLLM(SparQLeBase):
    @classmethod
    ...
    def build_speech_model_hubertbase(cls, args, dictionary=None, embed_tokens=None):
        speech_encoder = AutoModel.from_pretrained("facebook/hubert-base-ls960", cache_dir=os.environ['CACHE'], token=os.environ['HF_TOKEN'])
    ...
    def build_speech_model_hubertlarge(cls, args, dictionary=None, embed_tokens=None):
        speech_encoder = AutoModel.from_pretrained('facebook/hubert-large-ll60k', cache_dir=os.environ['CACHE'], token=os.environ['HF_TOKEN'])
    ...
    def add_args(parser):
        ...
        parser.add_argument("--speech-encoder-model", type=str, default='hubertlarge')
    ...
    def __init__(
        ...
        self.llm_tokenizer = AutoTokenizer.from_pretrained(args.llm_model, use_fast=False, cache_dir=os.environ['CACHE'], token=os.environ['HF_TOKEN'])
        if self.llm_tokenizer.pad_token is None:
            self.llm_tokenizer.pad_token = self.llm_tokenizer.eos_token
        self.llm_model = LlamaForCausalLM.from_pretrained(args.llm_model, cache_dir=os.environ['CACHE'], token=os.environ['HF_TOKEN'])
    ...
    def forward(self, samples):
        ...
        if self.speech_encoder_model == 'hubertbase' or self.speech_encoder_model == 'hubertlarge':
        ...
        text_tokens = self.llm_tokenizer(text, return_tensors="pt", padding="longest",).to(speech.device)
        targets = text_tokens.input_ids
        
        if self.prompt:
            targets[:, : self.prompt_length] = -100  # do not apply loss to the prompt

        empty_targets = torch.ones(atts_llm.size(), dtype=torch.long).to(speech.device).fill_(-100)
        targets = torch.cat([empty_targets, targets], dim=1)

        inputs_embeds = self.llm_model.model.embed_tokens(text_tokens.input_ids)
    ...
    def generate(
        ...
        text_tokens = self.llm_tokenizer(prompt,return_tensors="pt",).to(speech.device)
        attention_mask = torch.cat([atts_llm, text_tokens.attention_mask], dim=1)
        
        inputs_embeds = self.llm_model.get_input_embeddings()(text_tokens.input_ids.to(torch.int)).to(speech.device)
        inputs_embeds = torch.cat([inputs, inputs_embeds],dim=1)
...
@register_model_architecture(model_name="sparqle_finetune_llm", arch_name="sparqle_finetune_llm")
