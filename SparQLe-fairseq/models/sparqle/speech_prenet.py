...
class SpeechEncoderPrenet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.num_updates = 0
        assert encoder_speech_prenet in ["conv", "linear"], encoder_speech_prenet
        feature_enc_layers = eval(conv_feature_layers)  # noqa
        ...
        self.feat2tar_ratio = (
            label_rates * feature_ds_rate / sample_rate
        )
        ...
        self.use_sinc_pos = use_sinc_pos
        ...
        if self.use_conv_pos:
            ...
            dropout = 0
            std = math.sqrt((4 * (1.0 - dropout)) / (conv_pos * encoder_embed_dim))
        ...
        if self.use_sinc_pos:
            self.embed_positions = PositionalEmbedding(
                max_speech_positions, encoder_embed_dim, self.padding_idx
            )
        if self.use_abs_pos:
            self.embed_positions = PositionalEmbedding(
                max_speech_positions, encoder_embed_dim, self.padding_idx, learned=True
            )
